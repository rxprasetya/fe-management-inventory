...
const StockOutForm = () => {
    ...
    const { mutate, isPending, isError, error } = useAppMutation<ApiResponse<StockOut>, ApiError, StockOut>({
        mutationFn: (values) => id ? updateStockOut(values, id) : createStockOut(values),
        redirectTo: "/stock-out",
        queryKey: ["stock-out"],
        onSuccess: (data) => {
            setResponseMessage({
    ...
    const onSubmit = (values: StockOut) => {
        mutate(values)
    }
